...
compare_cycles.m     ← bar chart comparing efficiency of all three cycles
...
Animation only:
matlab>> pidton_animation
Type otto, diesel, or dual when asked.

Parameters
Otto — otto_engine(P1, T1, V1, r, T3, gamma)
InputWhat it isP1Initial pressure (Pa)T1Initial temperature (K)V1Initial volume (m³)rCompression ratioT3Peak temperature (K)gammaSpecific heat ratio
...
matlabresult.P            % pressure at each state point (Pa)
...
Quick Example
matlabP1 = 101325;  T1 = 300;  V1 = 0.0005;
...
Animation — 5 Strokes
StrokePistonValves1 — IntakeTDC → BDCIN open2 — CompressionBDC → TDCboth closed3 — Heat additiondepends on cycleboth closed4 — Expansioncutoff point → BDCboth closed5 — ExhaustBDC → TDCEX open
...
Comparing All Three Cycles
matlab>> compare_cycles
Runs all three cycles with the same base conditions (r = 8, P1 = 101325 Pa, T1 = 300 K) and plots a bar chart of their thermal efficiencies side by side. Quick way to see how they stack up without tweaking anything manually.
